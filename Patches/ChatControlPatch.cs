    ...
    public static void Postfix(ChatController __instance)
    {
        if (Main.DarkTheme.Value)
        {
            ...
            if (QuickChatIcon == null)
                ...
                QuickChatIcon.sprite = Utils.LoadSprite("TOHEV.Resources.Images.DarkQuickChat.png", 100f);

            if (OpenBanMenuIcon == null)
                OpenBanMenuIcon = GameObject.Find("OpenBanMenuIcon")?.transform.GetComponent<SpriteRenderer>();
            else
                OpenBanMenuIcon.sprite = Utils.LoadSprite("TOHEV.Resources.Images.DarkReport.png", 100f);

            if (OpenKeyboardIcon == null)
                OpenKeyboardIcon = GameObject.Find("OpenKeyboardIcon")?.transform.GetComponent<SpriteRenderer>();
            else
                OpenKeyboardIcon.sprite = Utils.LoadSprite("TOHEV.Resources.Images.DarkKeyboard.png", 100f);
        }
        else
        {

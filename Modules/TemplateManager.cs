...
{
    private static readonly string TEMPLATE_FILE_PATH = "./TOHEV-DATA/template.txt";
    ...
    public static void CreateIfNotExists()
    {
        ...
        try
        {
            ...
            if (!Directory.Exists(@"TOHEV-DATA")) Directory.CreateDirectory(@"TOHEV-DATA");
            var defaultTemplateMsg = GetResourcesTxt($"TOHEV.Resources.Config.template.{fileName}.txt");
            if (!File.Exists(@"./TOHEV-DATA/Default_Teamplate.txt")) //default template
            {
                Logger.Warn("Creating Default_Template.txt", "TemplateManager");
                using FileStream fs = File.Create(@"./TOHEV-DATA/Default_Teamplate.txt");
            }
            File.WriteAllText(@"./TOHEV-DATA/Default_Teamplate.txt", defaultTemplateMsg); //overwriting default template
